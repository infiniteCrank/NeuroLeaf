...
(async () => {
    ...
    function evaluateRecallMRR(query, reference, qLabels, rLabels, k) {
    ...
    for (const seq of hiddenUnitSequences) {
        for (const act of activations) {
            ...
            console.log(`\n🔹 Testing config: hiddenUnits ${JSON.stringify(seq)}, activation ${act}`);
            ...
            const embeddingStore = records.slice(0, sampleSize).map((rec, i) => ({
            ...
            // Compute Recall@1 manually
            let hitsAt1 = 0;
            for (const query of queries) {
                const scores = reference.map(r => ({
                    label: r.metadata.label,
                    score: query.embedding.reduce((sum, qv, i) => sum + qv * r.embedding[i], 0)
                }));
                scores.sort((a, b) => b.score - a.score);
                if (scores[0].label === query.metadata.label) hitsAt1++;
            }
            const recallAt1 = hitsAt1 / queries.length;

            csvLines.push(`ELMChain_${seq.join("_")}_${act},${recallAt1.toFixed(4)},${results.recallAtK.toFixed(4)},${results.mrr.toFixed(4)}`);
        }
    }
