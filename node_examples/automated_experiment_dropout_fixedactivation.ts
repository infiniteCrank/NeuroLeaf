...
(async () => {
        ...
        for (const act of activations) {
            for (const dropout of dropouts) {
                for (let run = 1; run <= repeats; run++) {
                    console.log(`\n🔹 Testing config: ${seq.join("_")} activation ${act} dropout ${dropout} (Run ${run})`);

                    const elms = seq.map((h, i) =>
                        new ELM({
                            activation: act, // 👈 ALL layers use the same activation
                            hiddenUnits: h,
                            maxLen: 50,
                            categories: [],
                    ...
                    csvLines.push(
                        `ELMChain_${seq.join("_")}_${act}_dropout${dropout}_run${run},` +
    ...
    const filename = `automated_experiment_dropout_fixedactivation_${timestamp}.csv`;
